    ...
    render: function() {
        ...
        var directMessageItems = this.state.showDirectChannels.map(function(channel) {
            ...
            if (!channel.fake) {
                ...
                return (
					<li key={channel.name} className={active}><a className={"sidebar-channel " + titleClass} href={"/" + TeamStore.getCurrentUrlId() + "/channels/"+channel.name}><span className="status" dangerouslySetInnerHTML={{__html: statusIcon}} /> {badge}{channel.display_name}</a></li>
                );
            }
