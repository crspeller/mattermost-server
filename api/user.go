...
func CreateUser(user *model.User) (*model.User, *model.AppError) {
	...
	if result := <-Srv.Store.User().GetTotalUsersCount(); result.Err != nil {
		...
		if count <= 0 {
			user.Roles = model.R_SYSTEM_ADMIN.Id + " " + model.R_SYSTEM_USER.Id
		}
	}
...
func updateRoles(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if user_id != c.Session.UserId && !HasPermissionToTeamContext(c, team_id, model.P_MANAGE_ROLES) {
	...
	if model.IsInRole(new_roles, model.R_SYSTEM_ADMIN.Id) && !HasPermissionToContext(c, model.P_ASSIGN_SYSTEM_ADMIN_ROLE) {
	...
	if model.IsInRole(user.GetRawRoles(), model.R_SYSTEM_ADMIN.Id) && !HasPermissionToContext(c, model.P_ASSIGN_SYSTEM_ADMIN_ROLE) {
	...
	if len(team_id) > 0 {
		...
		if !HasPermissionToContext(c, model.P_MANAGE_SYSTEM) {
			...
			if model.IsInRole(new_roles, model.R_TEAM_ADMIN.Id) && !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_ROLES) {
				c.Err = model.NewLocAppError("updateRoles", "api.user.update_roles.team_admin_needed.app_error", nil, "")
				c.Err.StatusCode = http.StatusForbidden
				return
			}
...
func updateActive(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !isSelfDeactive && !HasPermissionToContext(c, model.P_MANAGE_SYSTEM) {
...
func PermanentDeleteUser(c *Context, user *model.User) *model.AppError {
	...
	if user.IsInRole(model.R_SYSTEM_ADMIN.Id) {
...
func ResetPassword(c *Context, userId, newPassword string) *model.AppError {
	...
	if user.AuthData != nil && len(*user.AuthData) != 0 && !HasPermissionToContext(c, model.P_MANAGE_SYSTEM) {
