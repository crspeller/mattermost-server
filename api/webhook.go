...
func createIncomingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_WEBHOOKS) {
		return
	}
	...
	if channel.Type != model.CHANNEL_OPEN && !HasPermissionToChannelContext(c, channel.Id, model.P_READ_CHANNEL) {
...
func deleteIncomingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Webhook().GetIncoming(id); result.Err != nil {
		...
		if c.Session.UserId != result.Data.(*model.IncomingWebhook).UserId && !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_OTHERS_WEBHOOKS) {
...
func deleteOutgoingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Webhook().GetOutgoing(id); result.Err != nil {
		...
		if c.Session.UserId != result.Data.(*model.OutgoingWebhook).CreatorId && !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_OTHERS_WEBHOOKS) {
			c.LogAudit("fail - inappropriate permissions")
...
func regenOutgoingHookToken(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Webhook().GetOutgoing(id); result.Err != nil {
		...
		if c.TeamId != hook.TeamId && c.Session.UserId != hook.CreatorId && !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_OTHERS_WEBHOOKS) {
			c.LogAudit("fail - inappropriate permissions")
