...
func executeCommand(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(channelId) > 0 {
		if !HasPermissionToChannelContext(c, channelId, model.P_USE_SLASH_COMMANDS) {
			return
		}
	}
...
func createCommand(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_SLASH_COMMANDS) {
...
func regenCommandToken(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Command().Get(id); result.Err != nil {
		...
		if c.TeamId != cmd.TeamId || (c.Session.UserId != cmd.CreatorId && !HasPermissionToCurrentTeamContext(c, model.P_MANAGE_OTHERS_SLASH_COMMANDS)) {
...
func deleteCommand(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Command().Get(id); result.Err != nil {
		...
		if c.TeamId != result.Data.(*model.Command).TeamId || (c.Session.UserId != result.Data.(*model.Command).CreatorId && HasPermissionToCurrentTeamContext(c, model.P_MANAGE_OTHERS_SLASH_COMMANDS)) {
			c.LogAudit("fail - inappropriate permissions")
