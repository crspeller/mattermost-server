...
func uploadFile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	for i, _ := range files {
		...
		fileUrl := c.SiteUrl + "/api/v1/files/get/" + channelId + "/" + c.Session.UserId + "/" + uid + "/" + url.QueryEscape(files[i].Filename)
...
func getPublicLink(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("%s/api/v1/files/%s/%s/%s/%s?d=%s&h=%s&t=%s", c.SiteUrl, getType, channelId, userId, filename, url.QueryEscape(data), url.QueryEscape(hash), c.Session.TeamId)
